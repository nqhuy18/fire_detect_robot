...
void Drive_VW(Motor *mL, Motor *mR, float v_mps, float w_radps)
{
    ...
    float vL = v_mps - w_radps * half_track;   // [m/s]
    float vR = v_mps + w_radps * half_track;   // [m/s]

    double rpsL = (double)(vL / two_pi_R);
    double rpsR = (double)(vR / two_pi_R);
    ...
    Motor_SetTarget(mL, rpsL);
    Motor_SetTarget(mR, rpsR);
}
